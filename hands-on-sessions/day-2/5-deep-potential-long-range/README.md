...
~~~bash
cd workshop-july-2022/hands-on-sessions/day-2/DPLR/train/ 
~~~
`ls` you will see the files in this folder, where`input.json` is the input training file.  `cat input.json` to read this file. You will find that the training of the DPLR model is very similar to the training of a standard short-range DP models. Compared to DP, DPLR has an additional block "modifier" as shown below.
...
qsub job.sh
~~~
The training takes about 20 hours using one GPU. After the training is finished, we can freeze the model using `dp freeze -o model.pb`

## Comparing DP with DPLR
...
![Snipaste_2022-06-30_22-05-55](https://user-images.githubusercontent.com/101140941/176809175-0d69f612-40b7-4f2b-86c4-5d7937eb0125.png)


First, let's go to the water-dimer folder
~~~bash
cd workshop-july-2022/hands-on-sessions/day-2/DPLR/water-dimer
~~~
...
In the folder `workshop-july-2022/hands-on-sessions/day-2/DPLR/water-dimer/dp`, the file `dp.data.out` is the results predicted by dp.

Now, let's plot these results in one picture for comparison. 
~~~bash
cd workshop-july-2022/hands-on-sessions/day-2/DPLR/water-dimer/predict
gnuplot 
load "plot.plt"
~~~
...
## DPLR molecular dynamics simulation
...
~~~bash
cd workshop-july-2022/hands-on-sessions/day-2/DPLR/md
~~~
...
Finally, we run the simulation by submitting the job using `qsub job.sh`. You will get `water.dump` (trajectory file) and `log.lammps`.
